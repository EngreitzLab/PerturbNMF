...
#SBATCH --job-name=test_torch_minibatch
...
LOG_DIR="tests/output/torch-cNMF/minibatch/Inference/logs"
mkdir -p "$LOG_DIR"

eval "$(conda shell.bash hook)"
conda activate torch-nmf-dl

echo "=== torch-cNMF minibatch mode test ==="
echo "Node: $(hostname)"
...
python -m pytest tests/Script/Inference/test_inference_torch.py -v --tb=short \
    -k "TestDataValid or TestMinibatchMode" 2>&1 | tee "${LOG_DIR}/test_minibatch.out"

echo ""
echo "=== minibatch mode test done ==="
